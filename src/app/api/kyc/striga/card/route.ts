import { getCardValidationSchema } from "@/backend/Striga/CardValidationSchema";
...
const ALLOWED_ENDPOINTS = [
  ...
  "/card",
  "/card/report-missing",
  "/card/request-consent",
  "/card/confirm-consent",
  "/card/statement",
  "/card/activate",
...
export async function POST(req: NextRequest) {
  try {
    ...
    await getCardValidationSchema(endpoint)?.validate(data);
    if (method === "GET" && endpoint === "/card") {
      _endpoint = `/card/${data.cardId}`;
      _data = {};
    }
