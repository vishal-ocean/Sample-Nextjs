...
export const AssetsDropdown = () => {
  ...
  const { AssetsDropdownItems } = useYieldStaticData();

  return (
    <>
      <div
        className={cn(
          'px-4 py-2 rounded-full bg-secondary justify-center items-center hidden w-full cursor-pointer dark:bg-white/15',
          openAssetsDropdown &&
      ...
      >
        <div className="flex gap-x-2 items-center">
          {assets !== 'All Assets' &&
            AssetsDropdownItems.find((x) => x.assets === assets)?.assetsIcon}
          <span className="font-700 leading-4 text-14 ">
            {AssetsDropdownItems.find((x) => x.assets === assets)?.assets}
          </span>
        </div>
        <UilAngleDown
          className={cn(
      ...
      <DropdownMenu onOpenChange={setOpenAssetsDropdown}>
        <DropdownMenuTrigger className="block sm:w-[120px] w-full">
          <div
            className={cn(
              'px-4 py-2 rounded-full bg-secondary flex justify-center sm:justify-between items-center gap-2 dark:bg-white/15',
              openAssetsDropdown &&
          ...
          >
            <div className="flex gap-x-2 items-center">
              {assets !== 'All Assets' &&
                AssetsDropdownItems.find((x) => x.assets === assets)
                  ?.assetsIcon}
              <span className="font-700 leading-4 text-14 whitespace-nowrap">
                {AssetsDropdownItems.find((x) => x.assets === assets)?.assets}
              </span>
            </div>
            <UilAngleDown
        ...
        <DropdownMenuContent
          className={cn(
            'bg-gray-500/50 backdrop-blur-[16px] p-4 pr-3 rounded-[12px] bottom-0 w-[200px] dark:bg-gray-250/10'
          )}
          align="start"
        >
          <div>
