...
export const PhysicalDebitCard = () => {
  const { setHandleModal } = useHandleModalAction;
  const { setCardActionDetails } = useCardAction;

  return (
    <div className="bg-blue-990 rounded-[24px] w-full md:w-3/6 relative overflow-hidden group">
      <div className="absolute w-full bg-primary rounded-full h-24 blur-[200px] group-hover:h-[216px] "></div>
      ...
      <div className="relative rounded-[18px] w-[268px] h-40 md:w-80 lg:w-[402px] md:h-[200px] lg:h-[240px] m-auto -bottom-5 md:-bottom-9 group-hover:-bottom-2 transform transition-all duration-150 ease-in-out delay-0">
        ...
        />
        <SmallCardShape className="w-full h-full lg:hidden fill-primary" />
        <span className="absolute text-white font-500 md:leading-[30px] md:text-24 top-5 left-5 md:top-[30px] md:left-[30px] text-16 leading-5">
          Physical
        </span>
        <div className="lg:h-[60px] lg:w-[60px] h-10 w-10 flex items-center justify-center absolute top-0 right-0 bg-white/15 rounded-[300px]">
          <UilCreditCard className="w-4 h-4 lg:w-6 lg:h-6 text-white " />
        </div>
