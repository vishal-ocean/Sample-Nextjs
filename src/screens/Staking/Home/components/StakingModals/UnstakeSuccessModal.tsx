...
import { UNSTAKE_SUCCESS_MODAL } from "@/constants";
...
const UnstakeSuccessModal = () => {
  ...
  const handleOpenChange = (e: boolean) => {
    setHandleModal(e ? UNSTAKE_SUCCESS_MODAL : "");
  };
  ...
  return (
    <CustomModal
      ...
      open={modalOpen === UNSTAKE_SUCCESS_MODAL}
      ...
      {loading ? (
        ...
        <div className="text-center mx-auto mt-6 sm:mt-8 flex flex-col">
          ...
          <span className="text-24 font-500 text-blue-300 leading-[120%] dark:text-white">
            You unstaked {Number(selectedStake?.amount.toFixed(8))}{" "}
            {selectedStake?.assetName}
          </span>
          <span className="w-[269px] text-center self-center text-16 text-blue-300/60 font-500 mt-2 leading-[120%] dark:text-white/60">
            ...
            <Link href="/stakes" onClick={() => setHandleModal("")}>
              <span className="text-primary underline">Staking Dashboard</span>
            </Link>
          </span>
          <Button
            variant="secondary"
            className="mb-7 font-700 text-blue-300 mt-11 w-fit px-6 py-4 self-center leading-4 dark:text-white dark:bg-white/15"
